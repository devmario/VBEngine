...
unsigned char* PNGReader(FILE* _fp, long _pngSize, unsigned long* _w, unsigned long* _h, unsigned char* _color, unsigned char* _bit) {
	...
	*_bit = png_get_bit_depth(png_ptr, info_ptr);
	...
	if(png_get_color_type(png_ptr, info_ptr) == PNG_COLOR_TYPE_RGB)
		DebugPrintAbort(true, "파일의 칼라타입이 PNG_COLOR_TYPE_RGB 인데, 반드시 PNG_COLOR_TYPE_RGBA 타입이어야 합니다.");
	
	if(png_get_color_type(png_ptr, info_ptr) != PNG_COLOR_TYPE_RGBA)
		DebugPrintAbort(true, "파일의 칼라타입이 반드시 PNG_COLOR_TYPE_RGBA 타입이어야 합니다. PNG_COLOR_TYPE_RGBA = %d, 현제 칼라타입 = %d.", PNG_COLOR_TYPE_RGBA, png_get_color_type(png_ptr, info_ptr));
