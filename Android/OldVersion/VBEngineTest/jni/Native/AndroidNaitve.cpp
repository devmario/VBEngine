...
void nativeTouch(JNIEnv* env,jobject thiz, jint mode, jint id, jint cnt, jfloat x, jfloat y)
{
	
	VBTouchType _type;
	VBChar _tab_count = cnt;
	switch(mode)
	{
		case 0:
			_type = VBTouchType_Begin;
		break;
		case 1:
			_type = VBTouchType_Move;
		break;
		case 2:
			_type = VBTouchType_End;
		break;
		case 3:
			_type = VBTouchType_Cancel;
		break;
	}
	//LOGV("nativeTouch type:%d id:%d count:%d x:%f  y:%f",mode, id, cnt, x, y);
	EngineSampleTouch(sample, _type, NULL, _tab_count, VBVector2DCreate(x, y));
}
...
int registerNatives(JNIEnv* env)
{
	...
	static JNINativeMethod VBGLSurfaceViewMethods[] = {
	  ...
	  {"nativeTouch", "(IIIFF)V", (void*)nativeTouch},
