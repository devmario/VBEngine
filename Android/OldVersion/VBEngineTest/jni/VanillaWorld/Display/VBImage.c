...
void VBImageLoad(VBImage* _img, VBString* _path) {
	...
	if(_img) {
		...
		if(VBStringIsEqualToCString(_ext, "PNG")) {
			...
			VBPNGFileFree(&_png);
		} else if(VBStringIsEqualToCString(_ext, "JPEG") || VBStringIsEqualToCString(_ext, "JPG")) {
			VBJPEGFile* _jpeg = VBJPEGFileInitWithFilePath(VBJPEGFileAlloc(), _path);
			
			_img->color_type = VBJPEGFileGetColorType(_jpeg);
			_img->color_bit = VBJPEGFileGetColorBit(_jpeg);
			_img->width = VBJPEGFileGetWidth(_jpeg);
			_img->height = VBJPEGFileGetHeight(_jpeg);
			
			VBULong _size = VBJPEGFileGetDataSize(_jpeg);
			void* _data = VBJPEGFileGetData(_jpeg);
			
			_img->data = VBSystemMalloc(_size);
			memcpy(_img->data, _data, _size);
			
			VBJPEGFileFree(&_jpeg);
		} else {
#ifdef _VB_DEBUG_
			VBDebugPrintAndPrintLogFileAbort(VBEngineGetDefaultDebuger(), VBTrue, 
											 "VBEngine Log: VBImageLoad() - VBEngine이 지원하는 이미지 확장자가 아닙니다. PNG, JPEG, JPG를 이용하세요.(대소문자 무시)");
#endif
		}
...
#endif
	VBULong _i = _y;
	for(_i; _i < _y + _height; _i++) {
		VBULong _j = _x;
		for(_j; _j < _x + _width; _i++) {
