...
VBDrawable2D* VBDrawable2DInit(VBDrawable2D* _drawable) {
    ...
    if(_drawable->color_add)
        VBSystemFree(_drawable->color_add);
    _drawable->color_add = VBNull;
    
    if(_drawable->colorf)
        VBSystemFree(_drawable->colorf);
    _drawable->colorf = VBNull;
    
    if(_drawable->coloraddf)
        VBSystemFree(_drawable->coloraddf);
    _drawable->coloraddf = VBNull;
...
VBDrawable2D* VBDrawable2DInitWithStepSize(VBDrawable2D* _drawable) {
    ...
    if(VBEngineOpenGLGetIsSuportedShader() == VBFalse) {
        ...
        if(_drawable->color_add) {
            VBSize _size = VBColorTypeGetSize(VBColorType_RGBA) * _drawable->poly_stepSize;
            _drawable->color_add = VBSystemRealloc(_drawable->color_add, _size);
        } else {
            VBSize _size = VBColorTypeGetSize(VBColorType_RGBA) * _drawable->poly_stepSize;
            _drawable->color_add = VBSystemMalloc(_size);
        }
    } else {
        if(_drawable->colorf) {
            VBSize _size = sizeof(VBColorRGBAf) * _drawable->poly_stepSize;
            _drawable->colorf = VBSystemRealloc(_drawable->colorf, _size);
        } else {
            VBSize _size = sizeof(VBColorRGBAf) * _drawable->poly_stepSize;
            _drawable->colorf = VBSystemMalloc(_size);
        }
        
        if(_drawable->coloraddf) {
            VBSize _size = sizeof(VBColorRGBAf) * _drawable->poly_stepSize;
            _drawable->coloraddf = VBSystemRealloc(_drawable->coloraddf, _size);
        } else {
            VBSize _size = sizeof(VBColorRGBAf) * _drawable->poly_stepSize;
            _drawable->coloraddf = VBSystemMalloc(_size);
        }
    }
...
VBDrawable2D* VBDrawable2DInitWithData(VBDrawable2D* _drawable, VBDrawable2DType _draw_type,
    ...
    _drawable->color_add = VBSystemMalloc(_size);
    memset(_drawable->color_add, 0, _size);
...
VBDrawable2D* VBDrawable2DInitWithDataOnOpenGLES2(VBDrawable2D* _drawable, VBDrawable2DType _draw_type,
                                               VBTexture* _tex, VBUShort _poly_len, 
                                               VBColorRGBAf *_color, VBColorRGBAf *_color_add,
                                               VBVector2D* _vtx, VBVector2D* _txc, 
                                               VBUShort _idx_len, VBUShort* _idx) 
{
#ifdef _VB_DEBUG_
	if(_drawable == VBNull)
	...
	VBSize _size = sizeof(VBColorRGBAf) * _poly_len;
	_drawable->colorf = VBSystemMalloc(_size);
	memcpy(_drawable->colorf, _color, _size);
    
    _drawable->coloraddf = VBSystemMalloc(_size);
    memcpy(_drawable->coloraddf, _color_add, _size);
...
//        return;
    ...
    if(_drawable_1->useStepSize) {
        if(_drawable_1->poly_len > _drawable_1->poly_stepSize) {
            ...
            if(VBEngineOpenGLGetIsSuportedShader() == VBFalse) {
                ...
                _size = VBColorTypeGetSize(VBColorType_RGBA) * _drawable_1->poly_stepSize;
                _drawable_1->color_add = VBSystemRealloc(_drawable_1->color_add, _size);
            } else {
                _size = sizeof(VBColorRGBAf) * _drawable_1->poly_stepSize;
                _drawable_1->colorf = VBSystemRealloc(_drawable_1->colorf, _size);
                
                _size = sizeof(VBColorRGBAf) * _drawable_1->poly_stepSize;
                _drawable_1->coloraddf = VBSystemRealloc(_drawable_1->coloraddf, _size);
            }
        ...
        if(VBEngineOpenGLGetIsSuportedShader() == VBFalse) {
            ...
            _size = sizeof(VBColorRGBAf) * _drawable_1->poly_len;
            _drawable_1->colorf = VBSystemRealloc(_drawable_1->colorf, _size);
    ...
    if(VBEngineOpenGLGetIsSuportedShader() == VBFalse) {
        ...
        VBColorRGBA* _color_add = _drawable_1->color_add + _prev_poly_len;
        memcpy(_color_add, _drawable_2->color_add, VBColorTypeGetSize(VBColorType_RGBA) * _next_poly_len);    
    } else {
        VBColorRGBAf* _colorf = _drawable_1->colorf + _prev_poly_len;
        memcpy(_colorf, _drawable_2->colorf, sizeof(VBColorRGBAf) * _next_poly_len);
        
        VBColorRGBAf* _coloraddf = _drawable_1->coloraddf + _prev_poly_len;
        memcpy(_coloraddf, _drawable_2->coloraddf, sizeof(VBColorRGBAf) * _next_poly_len);
    }
...
void VBDrawable2DDraw(VBDrawable2D* _drawable) {
    ...
    if(VBEngineOpenGLGetIsSuportedShader() == VBFalse) {
        ...
        glVertexAttribPointer(ATTRIB_COLOR_PERCENT, 4, GL_FLOAT, GL_FALSE, 0, _drawable->colorf);
        glEnableVertexAttribArray(ATTRIB_COLOR_PERCENT);
       
        glVertexAttribPointer(ATTRIB_COLOR_ADD, 4, GL_FLOAT, GL_FALSE, 0, _drawable->coloraddf);
        glEnableVertexAttribArray(ATTRIB_COLOR_ADD);
        
        glVertexAttribPointer(ATTRIB_VERTEX, 2, GL_FLOAT, GL_FALSE, 0, _drawable->vtx);
        glEnableVertexAttribArray(ATTRIB_VERTEX);
            
        glVertexAttribPointer(ATTRIB_TEXCOORD, 2, GL_FLOAT, GL_FALSE, 0, _drawable->txc);
