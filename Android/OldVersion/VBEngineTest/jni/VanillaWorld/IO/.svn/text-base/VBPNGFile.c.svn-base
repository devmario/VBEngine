...
void VBPNGFileLoad(VBPNGFile* _png, VBString* _path) {
	...
	if(_png) {
		if(_png->loaded) {
			...
			VBFile* _file = (VBFile*) VBFileInitWithOpen(VBFileAlloc(),  _png->path,  VBFileMode_Read);
			if(_file == VBNull) {
#ifdef _VB_DEBUG_
				VBDebugPrintAndPrintLogFileAbort(VBEngineGetDefaultDebuger(), VBTrue, 
												 "VBEngine Log: VBPNGFileLoad() - PNG파일이 열리지 않습니다. 경로나 퍼미션을 확인해 주세요.");
#endif
            }
			VBUChar _header[8] = {0,};
			if(_file->file == NULL)
			fread(_header, sizeof(VBUChar), 8, _file->file);
			...
			png_init_io(_png_ptr, _file->file);
