...
Quaternion QuaternionLoadIdentity(void) {
	return QuaternionInit(0.0, 0.0, 0.0, 1.0);
}
...
Quaternion QuaternionInitAxisAngle(Vector3D axis, float angle) {
	...
	return QuaternionInit(axis.x * sinAngle, axis.y * sinAngle, axis.z * sinAngle, cos(angle));
}
...
Matrix3D QuaternionGetMatrix3D(Quaternion quaternion) {
	Matrix3D matrix;
	matrix.m11 = 1.0 - (2.0 * ((quaternion.y * quaternion.y) + (quaternion.z * quaternion.z)));
	matrix.m12 =       (2.0 * ((quaternion.x * quaternion.y) + (quaternion.z * quaternion.w)));
	matrix.m13 =       (2.0 * ((quaternion.x * quaternion.z) - (quaternion.y * quaternion.w)));
	matrix.m14 = 0.0;
	matrix.m21 =       (2.0 * ((quaternion.x * quaternion.y) - (quaternion.z * quaternion.w)));
	matrix.m22 = 1.0 - (2.0 * ((quaternion.x * quaternion.x) + (quaternion.z * quaternion.z)));
	matrix.m23 =       (2.0 * ((quaternion.y * quaternion.z) + (quaternion.x * quaternion.w)));
	matrix.m24 = 0.0;
	matrix.m31 =       (2.0 * ((quaternion.x * quaternion.z) + (quaternion.y * quaternion.w)));
	matrix.m32 =       (2.0 * ((quaternion.y * quaternion.z) - (quaternion.x * quaternion.w)));
	matrix.m33 = 1.0 - (2.0 * ((quaternion.x * quaternion.x) + (quaternion.y * quaternion.y)));
	matrix.m34 = 0.0;
	matrix.m41 = 0.0;
	matrix.m42 = 0.0;
	matrix.m43 = 0.0;
...
Quaternion QuaternionSlerp(Quaternion start, Quaternion end, float alpha) {
	...
	if(1.0 - fabs(difference) > QUATERNION_SLERP_TO_LERP_SWITCH_THRESHOLD) {
		float theta = acos(fabs(difference));
		float oneOverSinTheta = 1.0 / sin(theta);
