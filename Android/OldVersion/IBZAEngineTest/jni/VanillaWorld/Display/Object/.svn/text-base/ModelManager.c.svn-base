...
#pragma mark -
#pragma mark ModelManager 愿�━����� 醫���⑥�

bool ModelManagerStart(ModelManager* manager, unsigned int width,
		unsigned int height, ColorRGBA clearColor) {
...
#pragma mark -
#pragma mark ModelManager 愿�━������⑥�

bool ModelManagerSetScreenSize(ModelManager* manager, unsigned int width,
		unsigned int height) {
	manager->width = width;
	...
	manager->multy3D
			= Matrix3DMultiply(manager->projection3D, manager->model3D);
...
bool ModelManagerSetClearColor(ModelManager* manager, ColorRGBA clearColor) {
	...
	glClearColor(clearColor.r / 255.0, clearColor.g / 255.0, clearColor.b
			/ 255.0, clearColor.a / 255.0);
	return true;
}

bool ModelManagerSetClearColorHex(ModelManager* manager,
		ColorHexRGBA clearColor) {
...
#pragma mark -
#pragma mark ModelManager 愿�━��洹몃━湲��⑥�
int testCull;
bool ModelManagerDisplayBegin(ModelManager* manager,
		ModelManagerDisplayMode displayMode, Camera* camera, Light* light) {
	...
	if (manager->displayMode != displayMode) {
		...
		if (displayMode == ModelManagerDisplayMode3D) {
			...
			glOrthof(0, (float) manager->width, (float) manager->height, 0,
					1.0, 30000.0);
			// invert the y axis, down is positive
			glScalef(1, -1, 1);
			// mover the origin from the bottom left corner
			// to the upper left corner
			glTranslatef((float) manager->width * 0.5, -(float) manager->height
					* 0.5, 0);
		}
	}
	...
	t = (float) sqrt(manager->g_frustumPlanes[0][0]
			* manager->g_frustumPlanes[0][0] + manager->g_frustumPlanes[0][1]
			* manager->g_frustumPlanes[0][1] + manager->g_frustumPlanes[0][2]
			* manager->g_frustumPlanes[0][2]);
	...
	t = (float) sqrt(manager->g_frustumPlanes[1][0]
			* manager->g_frustumPlanes[1][0] + manager->g_frustumPlanes[1][1]
			* manager->g_frustumPlanes[1][1] + manager->g_frustumPlanes[1][2]
			* manager->g_frustumPlanes[1][2]);
	...
	t = (float) sqrt(manager->g_frustumPlanes[2][0]
			* manager->g_frustumPlanes[2][0] + manager->g_frustumPlanes[2][1]
			* manager->g_frustumPlanes[2][1] + manager->g_frustumPlanes[2][2]
			* manager->g_frustumPlanes[2][2]);
	...
	t = (float) sqrt(manager->g_frustumPlanes[3][0]
			* manager->g_frustumPlanes[3][0] + manager->g_frustumPlanes[3][1]
			* manager->g_frustumPlanes[3][1] + manager->g_frustumPlanes[3][2]
			* manager->g_frustumPlanes[3][2]);
	...
	t = (float) sqrt(manager->g_frustumPlanes[4][0]
			* manager->g_frustumPlanes[4][0] + manager->g_frustumPlanes[4][1]
			* manager->g_frustumPlanes[4][1] + manager->g_frustumPlanes[4][2]
			* manager->g_frustumPlanes[4][2]);
	...
	t = (float) sqrt(manager->g_frustumPlanes[5][0]
			* manager->g_frustumPlanes[5][0] + manager->g_frustumPlanes[5][1]
			* manager->g_frustumPlanes[5][1] + manager->g_frustumPlanes[5][2]
			* manager->g_frustumPlanes[5][2]);
	...
	int i = 0;
	for (; i < manager->touchArguments->length; i++) {
		ModelEventTouchArgument* touchArgument = ArrayObjectAtIndex(
				manager->touchArguments, i);

		float div = manager->multy3D.m33 == 0 ? 1.0 : manager->multy3D.m33;

		Vector4D screenVector = Vector4DInit((((touchArgument->touchVector.x
				- manager->view[0]) / manager->view[2]) * 2) - 1,

		(((manager->view[3] - touchArgument->touchVector.y - manager->view[1])
				/ manager->view[3]) * 2) - 1,
		...
		Vector4D object4D = Matrix3DMultiplyVector4D(manager->multy3D,
				screenVector);
		Vector3D object3D;
		if (object4D.w != 0.0) {
			object3D = Vector3DInit(object4D.x / object4D.w, object4D.y
					/ object4D.w, object4D.z / object4D.w);
		} else {
			object3D = Vector3DInit(0.0, 0.0, 0.0);
		}

		touchArgument->origin = Vector3DInit(0.0, 0.0, 0.0);
		touchArgument->direction = Vector3DSubtract(object3D,
				touchArgument->origin);
...
bool ModelManagerDisplay(ModelManager* manager, Model* model) {
	if (model->ani) {
		...
		if (model->ani->aniIsUpdateFrame) {
			...
			if (model->ani->aniChildLength) {
				for (i = 0; i
						< model->ani->aniChildLength[model->ani->aniCurFrame]; i++) {
					ModelAddAt(model,
							model->ani->aniChild[model->ani->aniCurFrame][i], 0);
					if (!model->ani->aniChild[model->ani->aniCurFrame][i]->ani->aniAbleUserControlMatrix)
						ModelSetMatrix(
								model->ani->aniChild[model->ani->aniCurFrame][i],
								model->ani->aniMat[model->ani->aniCurFrame][i]);
					if (!model->ani->aniChild[model->ani->aniCurFrame][i]->ani->aniAbleUserControlColor)
						ModelSetColor(
								model->ani->aniChild[model->ani->aniCurFrame][i],
								model->ani->aniColor[model->ani->aniCurFrame][i]);
				}
			}
	...
	if (model->isVisible) {
		...
		model->globalPosition = Matrix3DMultiplyVector3D(*model->matrixGlobal,
		...
		if (model->vertex) {
			...
			for (i = 0; i < 6; i++) {
				if (manager->g_frustumPlanes[i][0] * model->globalPosition.x
						+ manager->g_frustumPlanes[i][1]
								* model->globalPosition.y
						+ manager->g_frustumPlanes[i][2]
								* model->globalPosition.z
						+ manager->g_frustumPlanes[i][3]
						<= -model->globalRadius) {
		...
		if (model->isRender) {
			...
			if (model->touchEvent.event) {
				int i = 0;
				//LOGV("Model Touch Event Start %i " ,  manager->touchArguments->length);
				for (i = 0; i < manager->touchArguments->length; i++) {

					ModelEventTouchArgument* touchArgumentCurrent =
							ArrayObjectAtIndex(manager->touchArguments, i);

					bool hit = ModelGetIsVertexHitLineDirection(model,
							touchArgumentCurrent->origin,
							touchArgumentCurrent->direction,
							model->touchVector, model->touchVectorGlobal);

					model->touchEvent.argument.hitVector = model->touchVector;
					model->touchEvent.argument.hitVectorGlobal
							= model->touchVectorGlobal;
					model->touchEvent.argument.isHit = hit;
					model->touchEvent.argument.model = model;
					model->touchEvent.argument.touchID
							= touchArgumentCurrent->touchID;
					model->touchEvent.argument.touchTabCount
							= touchArgumentCurrent->touchTabCount;
					model->touchEvent.argument.touchType
							= touchArgumentCurrent->touchType;

					model->touchEvent.event(model->touchEvent.userReference,
							model->touchEvent.argument);

					touchArgumentCurrent->isChecked = true;
				}
				//LOGV("Model Touch Event End");
			}

			Model* _parent = ModelGetParent(model);
			int i = 0;
			for (i = 0; i < model->mixColorLength; i++) {
				if (_parent) {
					model->mixColor[i].r
							= (unsigned char) (((float) _parent->mixColor[0].r
									/ (float) 0xFF) * (float) model->color[i].r);
					model->mixColor[i].g
							= (unsigned char) (((float) _parent->mixColor[0].g
									/ (float) 0xFF) * (float) model->color[i].g);
					model->mixColor[i].b
							= (unsigned char) (((float) _parent->mixColor[0].b
									/ (float) 0xFF) * (float) model->color[i].b);
					model->mixColor[i].a
							= (unsigned char) (((float) _parent->mixColor[0].a
									/ (float) 0xFF) * (float) model->color[i].a);
				} else {
			...
			if (model->vertex) {
				...
				//glDisableClientState(GL_COLOR_ARRAY);
				glColor4ub(model->mixColor->r, model->mixColor->g,
						model->mixColor->b, model->mixColor->a);
...
bool ModelManagerDisplayEnd(ModelManager* manager) {
	int i = 0;
		for(; i < manager->touchArguments->length; ) {
			ModelEventTouchArgument* touchArgument = ArrayObjectAtIndex(manager->touchArguments, i);
			touchArgument->isChecked = true;
			if((touchArgument->touchType == TouchTypeEnd || touchArgument->touchType == TouchTypeCancel) || touchArgument->isDouble) {
				ArrayPopObjectAtIndex(manager->touchArguments, i);
				free(touchArgument);
			} else {
				i++;
			}
...
#pragma mark -
#pragma mark ModelManager 愿�━������댁� �대깽��諛�� �⑥�

bool ModelManagerTouch(ModelManager* manager, Vector2D touch, void* touchID,
		TouchType touchType, unsigned char tabCount) {

	ModelEventTouchArgument* touchArgument = (ModelEventTouchArgument*) calloc(
			1, sizeof(ModelEventTouchArgument));
...
bool ModelManagerTouchDown(ModelManager* manager, Vector2D touch,
		void* touchID, TouchType touchType, unsigned char tabCount) {
...
bool ModelManagerTouchMove(ModelManager* manager, Vector2D touch,
		void* touchID, TouchType touchType, unsigned char tabCount) {

	int len = manager->touchArguments->length;
			int i = 0;
			for(; i < len; i++) {
				ModelEventTouchArgument* touchArgument = ArrayObjectAtIndex(manager->touchArguments, i);
				if(touchArgument->touchID == touchID) {
					if(touchArgument->isChecked) {
						touchArgument->touchVector = touch;
						touchArgument->touchType = touchType;
						touchArgument->touchTabCount = tabCount;
					}
...
bool ModelManagerTouchUp(ModelManager* manager, Vector2D touch, void* touchID,
		TouchType touchType, unsigned char tabCount) {
		int len = manager->touchArguments->length;
			int i = 0;
			for(; i < len; i++) {
				ModelEventTouchArgument* touchArgument = ArrayObjectAtIndex(manager->touchArguments, i);
				if(touchArgument->touchID == touchID) {
						touchArgument->isDouble = true;

						ModelEventTouchArgument* newtouchArgument = (ModelEventTouchArgument*)calloc(1, sizeof(ModelEventTouchArgument));
						newtouchArgument->touchVector = touch;
						newtouchArgument->touchID = touchID;
						newtouchArgument->touchType = touchType;
						newtouchArgument->touchTabCount = tabCount;
						newtouchArgument->isDouble = true;
						ArrayPushObject(manager->touchArguments, newtouchArgument);
					}
			}
	return true;
}
