...
VBObjectFile2DKeyFrameElementGraphic* VBObjectFile2DKeyFrameElementGraphicInit(VBObjectFile2DKeyFrameElementGraphic* _graphic) {
    ...
    _graphic->color = VBColorRGBACreate(0xFF, 0xFF, 0xFF, 0xFF);
    ...
    if(_transition && _next) {
        ...
        *_matrix = VBMatrix2DWrapperLoadIdentity();
        if(VBObjectFile2DTransitionGetType(_transition) == VBObjectFile2DTransitionType_SingleBezier) {
            ...
            *_color = VBColorRGBAInterpolate(_origin_color, _next_color, _value);
...
VBObjectFile2DKeyFrameElementMovieClip* VBObjectFile2DKeyFrameElementMovieClipInit(VBObjectFile2DKeyFrameElementMovieClip* _movie_clip) {
    ...
    _movie_clip->color = VBColorRGBACreate(0xFF, 0xFF, 0xFF, 0xFF);
...
void VBObjectFile2DKeyFrameElementMovieClipGetTransitionProperties(VBObjectFile2DKeyFrameElementMovieClip* _movie_clip, VBFloat _t, VBMatrix2DWrapper* _matrix, VBColorRGBA* _color) {
    ...
    VBColorRGBA _origin_color = VBObjectFile2DKeyFrameElementMovieClipGetColor(_movie_clip);
    ...
    if(_transition && _next) {
        ...
        VBColorRGBA _next_color = VBObjectFile2DKeyFrameElementMovieClipGetColor(_next);
        *_matrix = VBMatrix2DWrapperLoadIdentity();
        if(VBObjectFile2DTransitionGetType(_transition) == VBObjectFile2DTransitionType_SingleBezier) {
            _value = VBObjectFile2DTransitionGetSingleValue(_transition, _t);
            *_matrix = VBMatrix2DWrapperInterpolate(_origin_matrix, _next_matrix, _value);
