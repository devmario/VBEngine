...
void VBModel::SetTexture(VBTexture* _tex) {
    ...
    this->setTextureRect( cocos2d::CCRectMake((_txc[0].x * tex->getPixelsWide()) / CCDirector::sharedDirector()->getContentScaleFactor()
                                              ...
                                              ,(_txc[3].x * tex->getPixelsWide() - _txc[0].x * tex->getPixelsWide()) / CCDirector::sharedDirector()->getContentScaleFactor()
                                              ,(_txc[3].y  * tex->getPixelsHigh() - _txc[0].y * tex->getPixelsHigh()) / CCDirector::sharedDirector()->getContentScaleFactor() ) );
}
...
bool VBModel::hitTest(VBModel* _ot) {
    ...
    while(remain < 0) {
        ...
        while(remain_ot < 0) {
            CCPoint vert_ot[4];
            double p_ot[4][2];
            remain_ot = _ot->getVertex(vertIdx_ot, vert_ot);
            vertIdx_ot++;
            if(remain_ot > -1)
                break;
            
            for(int i = 0; i < 4; i++) {
                p_ot[i][0] = (double)vert_ot[i].x;
                p_ot[i][1] = (double)vert_ot[i].y;
            }
            
            if(tri_tri_overlap_test_2d(p[0], p[1], p[2], p_ot[0], p_ot[1], p_ot[2])) {
                return true;
            }
            if(tri_tri_overlap_test_2d(p[0], p[1], p[2], p_ot[2], p_ot[1], p_ot[3])) {
                return true;
            }
            if(tri_tri_overlap_test_2d(p[2], p[1], p[3], p_ot[0], p_ot[1], p_ot[2])) {
                return true;
            }
            if(tri_tri_overlap_test_2d(p[2], p[1], p[3], p_ot[2], p_ot[1], p_ot[3])) {
                return true;
            }
        }
    }
